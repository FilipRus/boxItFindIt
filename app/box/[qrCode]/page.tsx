    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
        <div className="text-center bg-white rounded-lg border border-gray-200 p-8 max-w-md">
          ...
          <h1 className="text-xl font-semibold text-gray-900 mb-2">Box Not Found</h1>
          <p className="text-gray-500 text-sm">This QR code doesn&apos;t match any box in our system.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto px-3 sm:px-4 py-4 sm:py-6">
        ...
        {box.items.length === 0 ? (
          <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
            ...
            <p className="text-gray-400 text-sm">No items have been added yet.</p>
          </div>
        ) : viewMode === "grid" ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {box.items.map((item) => (
              <button
                ...
                className="bg-white border border-gray-200 rounded-lg hover:border-gray-300 hover:shadow-sm transition overflow-hidden text-left"
              >
                {item.imagePath ? (
                ...
                <div className="p-3">
                  ...
                  {item.category && (
                    <span className="inline-block bg-gray-100 text-gray-500 text-xs px-1.5 py-0.5 rounded mb-1">
                      {item.category}
                    </span>
                  )}
                  {item.description && (
          ...
          <div className="space-y-2">
            {box.items.map((item) => (
              <button
                key={item.id}
                onClick={() => setSelectedItem(item)}
                className="w-full bg-white border border-gray-200 rounded-lg hover:border-gray-300 transition p-4 flex gap-4 text-left"
              >
                {item.imagePath ? (
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-medium text-gray-900 text-sm mb-0.5">{item.name}</h3>
                  {item.category && (
                    <span className="inline-block bg-gray-100 text-gray-500 text-xs px-1.5 py-0.5 rounded mb-0.5">
                      {item.category}
                    </span>
                  )}
                  {item.description && (
          ...
          >
            ...
            <div className="p-6">
              ...
              {selectedItem.category && (
                <span className="inline-block bg-gray-100 text-gray-500 text-xs px-2 py-1 rounded mb-3">
                  {selectedItem.category}
                </span>
              )}
