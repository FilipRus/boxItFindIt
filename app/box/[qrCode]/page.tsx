          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {box.items.map((item) => (
              <button
                ...
                className={`border rounded-lg hover:shadow-sm transition overflow-hidden text-left ${
                  item.inUse
          ...
          <div className="space-y-2">
            {box.items.map((item) => (
              <button
                key={item.id}
                onClick={() => setSelectedItem(item)}
                className={`w-full border rounded-lg transition p-4 flex gap-4 text-left ${
                  item.inUse
                    ? "bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-800"
                    : "bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-700"
                }`}
            ...
            <div className="p-6">
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  {selectedItem.inUse && (
                    <span className="flex-shrink-0 bg-amber-100 dark:bg-amber-900/50 text-amber-700 dark:text-amber-400 text-xs font-medium px-2 py-1 rounded-full">In use</span>
                  )}
                </div>
                <button
                  onClick={() => setSelectedItem(null)}
                  className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition flex-shrink-0 ml-2"
                >
